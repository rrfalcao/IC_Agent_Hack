...
This is a Bun HTTP agent with `@ax-llm/ax` integration for LLM-powered capabilities. It includes payment support via x402 and uses `@aweto-agent/core` for agent app creation.
...
```typescript
import { createAxLLMClient } from "@aweto-agent/core";
