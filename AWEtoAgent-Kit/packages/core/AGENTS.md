...
`@aweto-agent/agent-auth` and reuse its `AgentRuntime` helpers instead of the
...
- `CreateAgentAppReturn<TApp>`: `{ app: TApp, runtime, agent, addEntrypoint, config }` (generic type in `@aweto-agent/types/core`)
